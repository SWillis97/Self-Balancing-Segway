import pyb
from pyb import LED, Pin, Timer
from oled_938 import OLED_938
from mpu6050 import MPU6050


### Motor Control

def init_motors():
    A1 = Pin('X3', Pin.OUT_PP)
    A2 = Pin('X4', Pin.OUT_PP)
    PWMA = Pin('X1')

    B1 = Pin('X7', Pin.OUT_PP)
    B2 = Pin('X8', Pin.OUT_PP)
    PWMB = Pin('X2')

    tim = Timer(2, freq=1000)
    motorA = tim.channel(1, Timer.PWM, pin=PWMA)
    motorB = tim.channel(2, Timer.PWM, pin=PWMB)
    return A1, A2, B1, B2, motorA, motorB

def init_oled():
    # I2C connected to Y9, Y10 (I2C bus 2) and Y11 is reset low active
    #OLED
    oled = OLED_938(pinout={'sda': 'Y10', 'scl': 'Y9', 'res': 'Y8'}, height=64,
                       external_vcc=False, i2c_devid=61)
    oled.poweron()
    oled.init_display()
    oled.draw_text(0,20,"Initializing")
    oled.display()
    return oled

def init_PWM():
    tim = Timer(2, freq=1000)
    #motorA = tim.channel(1, Timer.PWM, pin=PWMA)
    #motorB = tim.channel(2, Timer.PWM, pin=PWMB)

def forward(speed):
	A1.high()
	A2.low()
	B1.low()
	B2.high()
	motorA.pulse_width_percent(speed)
	motorB.pulse_width_percent(speed)

def backward(speed):
	A1.low()
	A2.high()
	B1.high()
	B2.low()
	motorA.pulse_width_percent(speed)
	motorB.pulse_width_percent(speed)

def initalize_robot():
    oled = init_oled()
    A1, A2, B1, B2, motorA, motorB = init_motors()

    # Define LEDs
    b_LED = LED(4)
    b_LED.toggle()
    # IMU connected to X9 and X10
    imu = MPU6050(1, False)    	 # Use I2C port 1 on Pyboard
    return imu, oled, A1, A2, B1, B2, motorA, motorB



def comp_filter(dt):
    global robot_pitch
    alpha = 0.7
    #pitch = int(imu.pitch())
    pitch = int(imu.pitch())
    roll = int(imu.roll())

    robot_pitch = alpha*(robot_pitch + imu.get_gy()*dt*0.001) + (1-alpha)*pitch
    # show graphics
    #oled.clear()
    #oled.draw_text(0,20," Raw | PITCH |: {}" .format(pitch))
    #oled.draw_text(0,40, "filtered: {}".format(robot_pitch))
    #oled.display()

def balance():
    #oled.clear()
    #forward(100)
    global robot_pitch


    if robot_pitch < 0:
        forward(abs(robot_pitch*4))
        #oled.draw_text(0,20,"forward: {}".format(robot_pit  ch))
    else:
        backward(abs(robot_pitch*4))
        #oled.draw_text(0,20,"backwards: {}".format(robot_pitch))

    #oled.display()

imu, oled, A1, A2, B1, B2, motorA, motorB = initalize_robot()


robot_pitch = 0
tic = pyb.millis()

while True:
    toc = pyb.millis()
    comp_filter(toc-tic)
    balance()
    #forward(100)
    tic = pyb.millis()
    #oled.clear()
    #oled.draw_text(0,20,"Running")
    #oled.display()
    b_LED.toggle()
